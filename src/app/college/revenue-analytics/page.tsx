...
export default function CollegeRevenuePage() {
  ...
  const fetchRevenueData = async () => {
    setLoading(true);
    try {
      const res = await axios.get("/api/college/revenue");
      if (res.data.success) {
      ...
      console.error("Error fetching college revenue data:", error);
  ...
  return (
    <div className="poppins space-y-6 pb-12">
      <Title
        title="College Revenue & Audits"
        subtitle="Detailed tracking of your college fests' fee collections, net payouts, and ledger lists."
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="card bg-base-200 border border-base-300 p-5 rounded-2xl shadow flex flex-row items-center justify-between">
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-base-content/40 uppercase tracking-widest block font-mono">Gross Collections</span>
            <span className="text-2xl font-extrabold text-success font-outfit">₹{stats.totalRevenue.toLocaleString("en-IN")}</span>
            <span className="text-[10px] text-success/80 flex items-center gap-0.5"><IconArrowUpRight size={12} /> +10.2% vs last semester</span>
          </div>
        ...
        <div className="card bg-base-200 border border-base-300 p-5 rounded-2xl shadow flex flex-row items-center justify-between">
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-base-content/40 uppercase tracking-widest block font-mono">Service Fee Cut (10%)</span>
            <span className="text-2xl font-extrabold text-error font-outfit">₹{stats.totalPlatformCut.toLocaleString("en-IN")}</span>
            <span className="text-[10px] text-base-content/40 block">Deducted automatically by portal</span>
          </div>
        ...
        <div className="card bg-base-200 border border-base-300 p-5 rounded-2xl shadow flex flex-row items-center justify-between">
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-base-content/40 uppercase tracking-widest block font-mono">Net College Payout</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={monthlyTrends}>
                <defs>
                  <linearGradient id="colorCollegeAmt" x1="0" y1="0" x2="0" y2="1">
                ...
                <Area type="monotone" dataKey="amount" stroke="var(--p, #ff8f00)" strokeWidth={2} fillOpacity={1} fill="url(#colorCollegeAmt)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-base-200 border border-base-300 p-5 rounded-2xl shadow-md flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-base text-base-content font-outfit uppercase mb-4">Payout Policy</h3>
            <p className="text-xs text-base-content/70 leading-relaxed">
              Ticket payouts are disbursed automatically into the college's verified bank account on a bi-monthly cycle. Platform commission is deducted at the time of transactions.
            </p>
          </div>
                ...
                {payments.map((p) => (
                  <tr key={p._id} className="hover:bg-base-300/10 text-xs border-b border-base-300/40">
                    ...
                    <td>
                      ...
                      <div className="text-[10px] text-base-content/40">{p.event?.title || "Fest name"}</div>
                    </td>
